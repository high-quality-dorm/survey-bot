...
from survio.db.database import get_session
...
class SurveyEngine:
    ...
    async def load_survey(self, survey: Path | str) -> str:
        ...
        return await self.survey_service.create_survey_from_json(survey_data,self.session)

    async def start_survey(self, survey_uuid: str) -> schemas.Question:
        self.question = await self.survey_service.get_first_question(survey_uuid, self.session)
    ...
    async def submit_answer(self, answer_id: int) -> schemas.Question | None:
        if self.user is None:
            await self.init()
        self.question = await self.survey_service.submit_answer(answer_id, self._user_id, self.session)
        return self.question
    ...
    async def get_survey_result(self, survey_uuid: str) -> schemas.SurveyResult:
        ...
        return await self.survey_service.get_survey_result(survey_uuid, self._user_id, self.session)

    async def get_all_results(self, survey_uuid: str) -> list[schemas.SurveyResult]:
        return await self.survey_service.get_all_survey_results(survey_uuid, self.session)

    async def get_user_passes(self, survey_id: int) -> list[schemas.Pass]:
        if self.user is None:
            await self.init()
        return await self.pass_service.get_user_passes(survey_id, self._user_id, self.session)